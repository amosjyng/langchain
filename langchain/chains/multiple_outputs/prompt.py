    ...
    def __init__(
        ...
        Args:
            prefix: Prompt that asks for the values of each of the variables the LLM
                    should fill in
            variables: A mapping from the display of each variable in the prompt to the
                    output key the variable should be stored in. Use this instead of
                    `variable_configs` if you don't care about customizing per-variable
                    prompts.
        ...
        if not variable_configs:
            ...
            variable_configs = [
                VariableConfig(display, output_key)
                for display, output_key in variables.items()
            ]

        if auto_suffix_variable_display and variable_configs:
            new_configs = []
